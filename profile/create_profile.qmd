...
execute:
  ...
  keep-md: false
params:
  jsonpath: "/Users/felix/Documents/Github/RESQUE/profile/data/resque_Felix.json"
---
...
credit_inv$CRediT_involvement <- "Low"
...
credit_inv$CRediT_involvement[credit_inv$sum_leadequal %in% c(3, 4)] <- "High"

credit_inv$CRediT_involvement[credit_inv$sum_equal %in% c(1, 2)] <- "Medium"
credit_inv$CRediT_involvement[credit_inv$sum_support >= 5] <- "Medium"

dat$CRediT_involvement <- factor(credit_inv$CRediT_involvement, levels=c("Very High", "High", "Medium", "Low"), ordered=TRUE)
...
```
From `r nrow(dat)` submitted papers of `r meta$LastName`, `r nrow(BIP %>% filter(pop_class <= "C4"))` `r ifelse(nrow(BIP %>% filter(pop_class <= "C4")) == 1, "was", "were")` in the top 10% popularity class of all papers or better.
