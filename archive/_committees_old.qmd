...
::: {.callout-warning}
The RESQUE Framework is constantly evolving. If you plan to use the RESQUE Tools in your hiring process, please get [in contact](team.qmd) with us first.
:::
...
This is another version that has been proposed:
...
Instead of putting the more technical information (such as the number of requested publications) in the official announcement, we recommend to add them to the welcome screen of your customized Collector App. This has the advantage that the text can be changed during the process (in contrast to the static text of the official announcement). Then a shorter paragraph in the announcement could be:

> Das Institut für Psychologie legt Wert auf transparente, belastbare und replizierbare Forschung. Daher ist in einem separaten Dokument darzulegen (ca. 1-2 Seiten), auf welche Art und Weise diese Ziele bisher verfolgt wurden und geplant ist, dies weiterhin zu tun (Open Science Statement). Ein Einbringen in die Open Science Initiative der Fakultät ist erwünscht. [...] Darüber hinaus bitten wir darum, ein ausgefülltes RESQUE-Profil, das in der RESQUE Collector App erstellt werden kann []ADD CUSTOM LINK], mit den anderen Bewerbungsunterlagen einzureichen. Die genauen Instruktionen finden sich in der App.
...
Here are some **exemplary job ads** that included RESQUE:

- Katholische Universität Eichstätt-Ingolstadt: [Professur für Entwicklungs- und Pädagogische
Psychologie (W3)](includes/job_ads/W3_Eichstaett_EntwicklungPaedagogischePsy.pdf)
- Johannes Gutenberg-Universität Mainz: [Juniorprofessur für Klinische Psychologie und Psychotherapie des Kindes- und Jugendalters (W1)](includes/job_W1_Mainz_KiJu.pdf.pdf)

## 5. Embedding RESQUE in your hiring portal

Applicants are required to enter their data into the Collector App and then download a `.json` file that contains all data.
This `.json` file then needs to be uploaded/sent to the committee.
Here are some experiences and caveats how to integrate this upload in existing hiring portals:
...
## 6. In the committee's meeting: Helping you to make good decisions with applicant profiles

We recommend that the weighting and importance of evaluative dimensions is discussed and fixed in written in the first session (within the provided RESQUE scoring categories, but also beyond, e.g. teaching, or outreach) In the first committee meeting

We provide **two types of research profile documents** that are designed to help committees make informed decisions:
