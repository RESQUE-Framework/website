...
score <- function(research_output, verbose=FALSE) {
    ...
    for (indicator_index in seq_along(scoring)) {
        if (verbose==TRUE) print(paste0("score() -> indicator_index = ", indicator_index))
        ...
        values <- map2_dbl(
            ...
            function(condition, value) {
                ev <- evaluate_condition_in_context(condition, research_output)
                if (!is.na(ev) & ev==TRUE) {
                    value
                } else {
                    0
...
score_all_from_file <- function(path) {
    ...
    score_all(research_outputs=research_outputs)
}
